...
class PeriodValidator:
    ...
    def __call__(self, value) -> None:
        ...
        if not 0 < int(temp_value) <= 7:
            raise ValidationError("Частота выполнения привычки не менее 1 и не более 7 раз в неделю")
    ...
    def __call__(self, value) -> None:
        if value.get("connected_habit"):
            ...
            if not connected_habit.is_pleasant:
                raise ValidationError("Нельзя сделать полезную привычку связанной")
...
class StartHabitValidator:
    ...
    def __call__(self, value) -> None:
        ...
        if temp_value <= now:
            raise ValidationError("Давайте лучше с завтрашнего дня!")
